<template>
  <div>
    ...
    <UModal
      :title="
        post?.id
          ? 'Post: ' + post?.title?.rendered || post?.title || 'not title found'
          : 'Add post'
      "
      :overlay="true"
...
watch(
  ...
  { immediate: true }
); // 'immediate: true' makes it run on initial component load
...
const modalCallback = (callbackData: {
  editedFields?: Record<
    string,
  ...
  savePost.value = Boolean(callbackData);
  if (callbackData) {
  }
};
</script>
